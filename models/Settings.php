...
class Settings extends \yii\db\ActiveRecord
{
    ...
    public function rules()
    {
        return [
            ...
            [['name'], 'unique'],
            ['header_menu','safe'],
//            ['header_menu', function ($attribute, $params) {
//                $valid = new UrlValidator();
//                if ($this->$attribute) {
////                    var_dump("<pre>",$this->$attribute);
//                    foreach ($this->$attribute as $key => $attr) {
//                        if (!$valid->validate($attr)) {
//                            $this->addError($attribute . "[" . $key . "]", 'Проверте правильно ли вы заполнили поле Социальные сети');
////                            var_dump("die");
//                        }
//                    }
////                    die();
...
//            }, 'skipOnEmpty' => false, 'skipOnError' => false],
    ...
    public function beforeSave($insert)
    {
        if (parent::beforeSave($insert)) {
            if ($this->name == "header_menu") {
                if (!@unserialize($this->header_menu)) {
                    $this->options = serialize($this->header_menu);
                }
                return true;
            } else {
                return false;
...
//            $this->header_menu = serialize($this->header_menu);
//            return true;
//        } else {
//            return false;
//        }
    ...
    {
        $file = file_get_contents(Yii::getAlias('@app').'/settings.json');
    ...
    public static function setValues($values)
    {
        ...
        file_put_contents(Yii::getAlias('@app').'/settings.json', json_encode($settings));
