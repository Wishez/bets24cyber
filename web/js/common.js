...
$(document).ready(function () {
    ...
    $("#showHideContent").click(function () {
        if ($("#content").is(":hidden")) {
            ...
            $(this).text("Развернуть переписку");
            $("#content").hide();
        }
        return false;
    });
    ...
    var dt = "March 10 2016 18:00:48";
    // var first = jsParseDate($(".clock").data('time')).date;
    ...
    /*$('#topStream').on('hidden', function () {
     $(this).remove();
     });*/

    /*$('body').on('hidden.bs.modal', '#topStream', function () {
     $(this).removeData('bs.modal');
     console.log("modal");
     });*/

    $('#topStream').on('hidden.bs.modal', function (e) {
        $(this).removeData('bs.modal');
        $(this).find('.site-stream').html('');
        // $(this).data('modal', null);
        console.log(this);
    });

    $('.m_left_game_list .aside-menu .aside-item h3').on('click', function (e) {
        console.log($(this).next('ul'));
        $(this).next('ul').toggleClass('vis')
    });

    if ($('div').is('#ww1')) {
        $("#ww1").eauth({"steam": {"popup": {"width": 990, "height": 615}}});
    }

    $('#myTabContent .legue-stream').on('click', function (e) {
        e.preventDefault();
        console.log($(this).data('stream'));
        $('.stream-video').html($(this).data('stream'))
    })
